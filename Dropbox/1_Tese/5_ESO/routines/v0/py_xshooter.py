...
sys.path.append('/scratch/home/sysadmin/.local/lib/python2.7/'
                'site-packages/lineid_plot.py to lineid_plot.pyc')
sys.path.append('/scratch/home/sysadmin/.local/lib/python2.7/'
                'site-packages/lineid_plot-0.3-py2.7.egg-info')
sys.path.append('/scratch/home/sysadmin/.local/lib/python2.7/'
                'site-packages/lmfit-0.9.3rc1_32_g9a6f751-py2.7.egg/lmfit/'
                '__init__.pyc')
sys.path.append('/scratch/home/sysadmin/.local/lib/python2.7/'
                'site-packages/lmfit-0.9.3rc1_32_g9a6f751-py2.7.egg/lmfit/'
                '__init__.py')
...
# ------------------------------------------------------------------------------
    ...
    for i in range(len(data)):
        ...
        if star_name == star and star != 'Hip093225':  # *** VOLTAR AQUI
...
# images and another to the data)
    ...
    if inputs == 'eso_demos':
        ...
        input_data_raw = '/scratch/home/sysadmin/Downloads/reflex_data/' + \
            'data_with_raw_calibs/'

    if inputs == 'eso':
        input_data = '/run/media/sysadmin/SAMSUNG/reduced/'
        input_data_raw = '/run/media/sysadmin/SAMSUNG/reflex_data/' + \
            'reflex_input/xshooter/'
    ...
    if plot_sed_lines is True:
        ...
        for i in range(len(list_stars)):
            if os.path.exists(folder_fig + list_stars[i]) is True and \
               os.path.exists(folder_fig + list_stars[i] + 'sed/') is True \
               and os.path.exists(folder_fig + list_stars[i] + 'bcd/') is True:
